...
public class DASASEEncryptUtil
{
     ...
     * DAS加密
     * 
     * @param decryptedText decryptedText
     * @return [参数说明]
     * 
     * @see [类�?�类#方法、类#成员]
     */
    ...
    private static String bytes2HexString(byte[] bytes)
    {
        ...
        if (bytes != null)
        {
            for (int bi = 0; bi < bytes.length; bi = bi + 1)
            {
                ...
                //hexString = hexString + (char)(((bytes[bi] >> NUMBER_FOUR) & 0x0F) + 'A');
                hexString.append((char)(((bytes[bi] >> NUMBER_FOUR) & 0x0F) + 'A'));
